...
if (!is_null(Core_Array::getGet('autocomplete')) && !is_null(Core_Array::getGet('queryString')))
{
	...
	if (strlen($sQuery))
	{
		...
		$oShop_Items = $oShop_Order->Shop->Shop_Items;
		...
		foreach ($aShop_Items as $oShop_Item)
		{
			...
			$fCurrencyCoefficient = $oShop_Item->Shop_Currency->id > 0 && $oShop_Order->Shop_Currency->id > 0
				? $oShop_Controller->getCurrencyCoefficientInShopCurrency(
					$oShop_Item->Shop_Currency,
					$oShop_Order->Shop_Currency
				)
				: 0;
			...
			$aJSON[] = array(
				...
				'currency' => $oShop_Order->Shop_Currency->name
			);
		}
	}
