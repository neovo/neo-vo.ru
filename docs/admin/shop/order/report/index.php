...
if(!is_null(Core_Array::getPost('do_show_report')))
{
	...
	if(count($aOrdersResult) > 0)
	{
				...
				<table  class="admin-table table table-bordered table-hover table-striped">
					<thead>
						<tr>
							...
							<th width="50"><?php echo Core::_('Shop_Item.form_sales_order_count_orders')?></th>
							<th width="50"><?php echo Core::_('Shop_Item.form_sales_order_count_items')?></th>
							<th width="100"><?php echo Core::_('Shop_Item.form_sales_order_total_summ')?></th>
							<th width="100"><?php echo Core::_('Shop_Item.form_sales_order_order_status')?></th>
						</tr>
					</thead>
		<?php
		...
		foreach ($aOrdersResult as $key => $rowOrdersResult)
		{
			...
			$oCategory->addAttribute('name', str_replace("&mdash;", "-", $period_title));
			$key % $iSkipItems != 0 && $oCategory->addAttribute('showName', 0);
			...
			$(function(){
				...
				var options = {
					...
					tooltipOpts: {
						...
						content: function(label, xval, yval, flotItem){return '<span>' + timePeriodTitles[xval] + '</span>, <b>' + label + '</b> : <span>' + yval + ' <?php echo $oShop->Shop_Currency->name ?></span>'; }
					}
				};
