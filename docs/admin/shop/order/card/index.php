...
<html>
<head>
<title><?php echo Core::_("Shop_Order.order_card_dt", $oShopOrder->invoice, Core_Date::sql2datetime($oShopOrder->datetime))?></title>
...
else
{
	...
	if ($oShopOrder->source_id)
	{
		?><tr><td colspan="2"></td></tr><?php

		$oSource = $oShopOrder->Source;
		...
		foreach ($aSourceFields as $sFieldName)
		{
			if ($oSource->$sFieldName != '')
			{
				?><tr>
					...
					<td><?php echo $oSource->$sFieldName?></td>
				</tr><?php
			}
		}
	}
