...
if ($iStructureId)
{
	$oAdmin_Form_Dataset
		...
		->addCondition(
			array(
				'where' => array('benchmark_urls.datetime', '>', date('Y-m-d :00:00', strtotime("-1 month")))
			)
		)
...
$oBenchmark_Url
	...
	->where('benchmark_urls.datetime', '>', date('Y-m-d :00:00', $iOneMonthAgo))
	->groupBy('benchmark_urls.datetime')
	->clearOrderBy()
	->orderBy('benchmark_urls.datetime', 'ASC');
...
$aTitlesPerDay = array();
$aLoadDomValuesPerDay = array();
$aLoadPageValuesPerDay = array();
$aDnsValuesPerDay = array();
$aServerValuesPerDay = array();
...
$oBenchmark_Url
	...
	->orderBy('HOUR(benchmark_urls.datetime)', 'ASC');
